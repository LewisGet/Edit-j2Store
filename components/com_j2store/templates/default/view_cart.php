...
?>
			<!-- Add to cart Successmsg -->
			<div id="j2store-add-to-cart-successmsg"></div>
				<?php if($this->item->inventory->can_allow || $this->item->inventory->backorder):?>
					<!-- product quantity -->
					<?php if($this->params->get('show_qty_field', 1) && $this->layout_params->get('product_show_qtybox', 1)):?>
	                 <div class="j2store-quantity">
		     		 	<div id='product_quantity_input_<?php echo $this->item->product_id; ?>' class="j2store-product-quantity-input">
							<span class="j2store-product-item-label">
								<?php echo $this->layout_params->get('product_qty_alternative_text', JText::_('J2STORE_ADDTOCART_QUANTITY'));?></span>
								<input
									 style="width:<?php echo $this->layout_params->get('show_qtybox_width', '30');?>px"
									 size="3"
					         		...
					         		<li>
					         			<?php $arrow_up =$this->layout_params->get('qty_plus_icon','fa fa-chevron-up') ;?>
					         			<i class="<?php echo $arrow_up;?>"  onclick="j2storeQtyPlus('add_to_cart_qty_input-<?php echo $this->item->product_id;?>')"></i>
						         	</li>
								    <li>
								    	<?php $arrow_down = $this->layout_params->get('qty_minus_icon','fa fa-chevron-down');?>
			...
			<!-- cart button -->
				...
				<span class="j2store-product-item-label" >
					<?php $cart_text = JText::_($this->layout_params->get('product_list_addtocart_alternative_text', 'J2STORE_ADD_TO_CART'));?>
				</span>
				<input id="addtoCartBtn" type="submit" class="j2store-item-cart-button button <?php echo $this->layout_params->get('add_to_cartbtn_class', ''); ?>" value="<?php echo $cart_text;?>" />
