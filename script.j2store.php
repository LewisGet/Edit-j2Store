...
class Com_J2StoreInstallerScript {
	...
	function preflight( $type, $parent ) {
		...
		if( version_compare( $jversion->getShortVersion(), '2.5.6', 'lt' ) ) {
			$app->enqueueMessage('Cannot install J2Store in a Joomla release prior to 2.5.6', 'error');
			return false;
		}
		...
		if(!function_exists('curl_init') || !is_callable('curl_init')) {
			$app->enqueueMessage('cURL library is not enabled in your PHP installation. Please contact your hosting service provider', 'error');
		}
		...
		if($result) {
			...
			if( version_compare( $version, '2.7.3', 'lt' ) ) {
				$app->enqueueMessage('You should first upgrade to J2Store 2.7.3 and then install the latest version. Otherwise, the changes made till 2.7.3 wont be reflected in your install', 'error');
				return false;
			}
		}
		$file = JPATH_ADMINISTRATOR.'/components/com_j2store/pre-version.txt';
		JFile::write($file, $version);
	...
	public function postflight($type, $parent)
	{
		...
		foreach ($plugins as $plugin)
		{
			...
			$publish = (string)$plugin->attributes()->publish;
	...
	private function _doDBChanges($type='') {
		...
		if(!in_array($prefix.'j2store_storeprofiles', $tables)){
			...
			CREATE TABLE IF NOT EXISTS `#__j2store_storeprofiles` (
			  ...
			  `store_default_layout` int(11) NOT NULL,
		...
		if(!in_array($prefix.'j2store_layouts', $tables)){
			$query ="
			CREATE TABLE IF NOT EXISTS `#__j2store_layouts` (
	...
	private function _modifyExistingTables($type='') {
		...
		if(in_array($prefix.'j2store_storeprofiles', $tables)){
			...
			if (!array_key_exists('store_default_layout', $fields)) {
				$query = "ALTER TABLE #__j2store_storeprofiles ADD `store_default_layout` int(11) NOT NULL AFTER `store_payment_layout`";
				$this->_executeQuery($query);
			}
		...
		if(in_array($prefix.'j2store_prices', $tables)){
			...
			if (!array_key_exists('main_image', $fields)) {
				$query = "ALTER TABLE #__j2store_prices ADD `main_image` text NOT NULL  AFTER `use_store_config_notify_qty`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('additional_image', $fields)) {
				$query = "ALTER TABLE #__j2store_prices ADD `additional_image` longtext NOT NULL  AFTER `main_image`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('listview_thumb', $fields)) {
				$query = "ALTER TABLE #__j2store_prices ADD `listview_thumb` text NOT NULL  AFTER `additional_image`";
				$this->_executeQuery($query);
			}
		...
		if(in_array($prefix.'j2store_orders', $tables)){
			...
			if (!array_key_exists('updated_at', $fields)) {
				$query = "ALTER TABLE #__j2store_orders ADD `updated_at` varchar(255) NOT NULL AFTER `created_date`";
				$this->_executeQuery($query);
			}
		...
		if(in_array($prefix.'j2store_coupons', $tables)){
			...
			if (!array_key_exists('free_shipping', $fields)) {
				$query = "ALTER TABLE #__j2store_coupons ADD `free_shipping` int(11) NOT NULL AFTER `max_value`";
				$this->_executeQuery($query);
			}
		...
		//order coupons
		if(in_array($prefix.'j2store_order_coupons', $tables)){
			$fields = $db->getTableColumns('#__j2store_order_coupons');

			if (!array_key_exists('coupon_code', $fields)) {
				$query = "ALTER TABLE #__j2store_order_coupons ADD `coupon_code` varchar(255) NOT NULL AFTER `coupon_id`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('order_id', $fields)) {
				$query = "ALTER TABLE #__j2store_order_coupons ADD `order_id` varchar(255) NOT NULL AFTER `orderpayment_id`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('customer_email', $fields)) {
				$query = "ALTER TABLE #__j2store_order_coupons ADD `customer_email` varchar(255) NOT NULL AFTER `customer_id`";
				$this->_executeQuery($query);
			}
	...
	private function _removeObsoleteModules() {
		$db = JFactory::getDbo();
		$query = "SELECT `extension_id` FROM `#__extensions` WHERE `type`='module' AND element = 'mod_j2store_quicktips'";
		$db->setQuery($query);
		$extensions = $db->loadColumn();
		if (count($extensions))
		{
