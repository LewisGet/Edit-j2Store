	...
	function __construct()
		...
		function saveAll($data)
		{
			...
			foreach($data['product'] as $value){
				...
				}elseif ($value['item_price'] > 0 || !empty($value['item_sku']) || $value['product_enabled'] >= 0) {
					if(isset($value['article_id']) && $value['article_id'] > 0) {
						$prices_row=JTable::getInstance('prices','Table');
						//try loading with the article_id. This helps avoid duplicate records
						$prices_row->load(array('article_id'=>$value['article_id']));
						$prices_row->bind($value);
						if(!$prices_row->check($value))
						{
							return false;
						}
