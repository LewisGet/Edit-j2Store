...
class J2StorePrices
{
	...
	public static function getPrice( $id, $quantity = '1')
	{
		...
		if ( !isset( $sets[$id][$quantity][$groups] ) )
		{
			...
			if(isset($group_price) && ($group_price)){
				...
				$item->product_price =(float) $group_price;
			}
	...
	public static function getPriceRange($product_id,$quantity)
	{
		...
		if ( !isset( $sets[$product_id][$quantity])) {
			...
			$db->setQuery($query);
			$sets[$product_id][$quantity] =$db->loadResult();
		}
		return $sets[$product_id][$quantity];
	}
...
*/
	...
	public static function getGroupPrice($product_id){
		...
		if ( !isset( $sets[$product_id][$groups])) {
			...
			//echo $query;
			$db->setQuery($query);
			$sets[$product_id][$groups] = $db->loadResult();
		}
		return $sets[$product_id][$groups];
	}
}
