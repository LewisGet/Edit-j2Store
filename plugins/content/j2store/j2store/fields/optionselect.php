	...
	function getInput(){
		...
		if(typeof(j2store.jQuery) == 'undefined') {
 		...
 		$product_id = $app->input->get('id');
				...
				$html .='</td></tr>';
				$html .='<tr><td colspan="3">';
				$html .=J2StorePopup::popup("index.php?option=com_j2store&view=products&task=setpaimport&tmpl=component&product_id={$product_id}", JText::_('J2STORE_IMPORT_PRODUCT_OPTIONS'), array('class'=>'btn btn-success'));
				$html .='</a>';
				$html .='</td></tr>';
				$html .='</tbody></table>';
		...
		if(count($pa_options)) {
			...
			foreach($pa_options as $pa_option) {
				...
				if($pa_option->type == 'select' || $pa_option->type == 'radio' || $pa_option->type == 'checkbox') {
					$html .= '&nbsp;&nbsp;<br />';
					$html .= J2StorePopup::popup( "index.php?option=com_j2store&view=products&task=setproductoptionvalues&product_option_id=".$pa_option->product_option_id."&tmpl=component", JText::_( "J2STORE_OPTION_SET_VALUES" ), array('class'=>'btn btn-primary'));
				}
				$html .='</td>';
				$html .='<td>';
