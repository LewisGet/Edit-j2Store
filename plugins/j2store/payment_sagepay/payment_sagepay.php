 ...
 function _cardTypesField( $field='cardtype', $default='', $options='' )
	...
	//	}
        ...
        if(!empty($orderinfo->billing_first_name)) {
        	...
        	$sage_state                 = substr($this->_getStateCode($orderinfo->billing_zone_id)->zone_code, 0, 2); //"TX";
        	...
        	//use shipping address
        {
        	...
        	$sage_state                 = substr($this->_getStateCode($orderinfo->shipping_zone_id)->zone_code, 0, 2); //TX
...
// basket format	4:Pioneer NSDV99 DVD-Surround Sound System:1:424.68:74.32:499.00: 499.00
	...
	$shipping_country = $this->_getCountryCode($orderinfo->shipping_country_id)->country_isocode_2;
        ...
        if($shipping_country == 'US') {
        	 $sagepay_values["DeliveryState"]= substr($this->_getStateCode($orderinfo->shipping_zone_id)->zone_code, 0, 2);
        }
   ...
   //voveran
	...
	function _getStateCode($zone_id) {
		$db = JFactory::getDBO();
		$query = $db->getQuery(true)->select('*')->from('#__j2store_zones')->where('zone_id='.$db->q($zone_id));
		$db->setQuery($query);
		return $db->loadObject();
	}
