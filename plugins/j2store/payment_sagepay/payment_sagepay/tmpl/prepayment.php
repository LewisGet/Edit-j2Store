...
function j2storeSagepayDirectSubmit(button) {
	...
	(function($) {
		...
		$(button).val('<?php echo JText::_('J2STORE_SAGEPAY_PROCESSING_PLEASE_WAIT')?>');
	...
	var jqXHR =	$.ajax({
		...
		jqXHR.done(function(json) {
			...
			if(json == null) {
				$(button).val('<?php echo JText::_('J2STORE_SAGEPAY_ERROR_PROCESSING')?>');
				form.find('.plugin_error_instruction').after('<br /><span class="j2error"><?php echo JText::_('J2STORE_SAGEPAY_ON_ERROR_INSTRUCTIONS'); ?></span>');
			}
			...
			if (json['redirect']) {
				$(button).val('<?php echo JText::_('J2STORE_SAGEPAY_COMPLETED_PROCESSING')?>');
